<script>
	import Footer from '$lib/Footer.svelte';
  	...
  	import InfinitSlide from '$lib/InfinitSlide.svelte';
	import TexxtSlider from '$lib/TexxtSlider.svelte';
	...
	$: isIleDeFrance = (() => {
		...
		return !isNaN(code) && code >= 75000 && code <= 75020 || code >= 93000 && code <= 93999 || code >= 94000 && code <= 94999 || code >= 77000 && code <= 77999 || code >= 95000 && code <= 95999 || code >= 78999&& code <= 78999 || code >= 92000 && code <= 92999 || code >= 91000 && code <= 91999;
	})();
...
<div class="flex w-full bg-gray-50 items-center justify-center pb-10 lg:py-10" id="test-elegibilite">
	...
	<div class="w-full sm:w-11/12 xl:w-10/12 2xl:w-7/12 grid grid-cols-1 lg:grid-cols-2 p-5 gap-3">
		<div class="flex flex-col gap-5">
			...
			<p class="text-center lg:text-left text-lg sm:text-xl">Savez-vous qu'en isolant entièrement votre maison, vous pouvez réaliser jusqu'à <span class="font-bold">70% d'économies </span> d'énergie ?</p>
			<div class="flex justify-end">
				<img src="arrow.png" alt="" class="hidden lg:block w-[80px] -mt-10 -mr-10 z-10">
			</div>
		</div>
		<div class="w-full bg-white p-5 border-2 border-green-500 shadow-xl shadow-gray-200 rounded-lg">
			<img src="./rge.png" class ="w-16 -mt-12" alt="Norme RGE">
			<h3 class=" sm:text-lg text-gray-500 text-center">Testez votre éligibilité en 30 secondes</h3>
			<form on:submit|preventDefault={() => console.log($formData)} class="flex flex-col items-center justify-center min-h-[400px]">
				{#if $currentStep === 1}
					...
					<ul class="grid w-full gap-3 md:grid-cols-2">
						<li>
							<input type="radio" id="hosting-small" name="habitation" value="Maison" bind:group={$formData.habitation} class="hidden peer" required />
							<label for="hosting-small" class="inline-flex items-center w-full p-5 text-gray-700 bg-white border border-gray-300 rounded-lg cursor-pointer peer-checked:border-green-600 peer-checked:text-green-600 hover:text-gray-600 hover:bg-gray-100 ">                           
								<svg  class="w-20" viewBox="0 0 75 75" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none">
								...
								<div class="block">
									<div class="w-full text-lg font-semibold">Maison</div>
									<div class="w-full text-xs">J'habite une maison</div>
								</div>
							</label>
						</li>
						<li>
							<input type="radio" id="hosting-big" name="habitation" value="Appartement" bind:group={$formData.habitation} class="hidden peer">
							<label for="hosting-big" class="inline-flex items-center gap-2 w-full p-5 text-gray-700 bg-white border border-gray-300 rounded-lg cursor-pointer  dark:peer-checked:text-green-500 peer-checked:border-green-600 peer-checked:text-green-600 hover:text-gray-600 hover:bg-gray-100 ">
								
								<svg class="w-20" viewBox="0 0 75 74.999997" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none">
								...
								<div class="block">
									<div class="w-full text-lg font-semibold">Appartement</div>
				...
				{#if $currentStep === 2}
					...
					<ul class="grid w-full gap-3 md:grid-cols-2">
						...
						<li>
							<input type="radio" id="owner" name="statut" value="Propriétaire" bind:group={$formData.statut} class="hidden peer">
							<label for="owner" class="inline-flex items-center gap-2 w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer  dark:peer-checked:text-green-500 peer-checked:border-green-600 peer-checked:text-green-600 hover:text-gray-600 hover:bg-gray-100 ">
								<svg class="w-20" viewBox="0 0 75 74.999997" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none">
								...
								<div class="block">
									<div class="w-full text-lg font-semibold">Propriétaire</div>
									<div class="w-full text-xs">Je suis Propriétaire</div>
								</div>
							</label>
						</li>
						<li>
							<input type="radio" id="locataire" name="statut" value="Locataire" bind:group={$formData.statut} class="hidden peer" required />
							<label for="locataire" class="inline-flex items-center w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-green-600 peer-checked:text-green-600 hover:text-gray-600 hover:bg-gray-100 ">                           

							<svg class="w-20" viewBox="0 0 75 74.999997" fill="none">
								...
								<div class="block">
									<div class="w-full text-lg font-semibold">Locataire</div>
				...
				{#if $currentStep === 3}
					...
					<div class="w-full flex flex-col" transition:fade>
						...
						<input type="text" class="input border px-5 py-1.5" bind:value={$formData.codePostal} placeholder="75000"/>
					</div>
				{/if}
						...
						{#if revenueOptions.length > 0}
							{#each revenueOptions as option , i}
									...
									<li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r ">
										<div class="flex items-center ps-3">
											<input id="{option}-{i}" type="radio" name="revenus" value={option} bind:group={$formData.revenus} class="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 focus:ring-green-500 ">
											<label for="{option}-{i}" class="w-full py-3 ms-2 text-sm font-medium text-gray-90">{option} </label>
										</div>
									</li>
								
							{/each}
						{:else}
						<p class="text-red-500">Veuillez d'abord entrer un code postal valide et sélectionner un foyer.</p>
						{/if}
					</ul>
				{/if}
				...
				{#if $currentStep === 8}
					...
					<div class="w-full flex flex-col">
						...
						<div class="w-full flex flex-col md:flex-row gap-2">
							<div class="relative">
								<div class="absolute inset-y-0 start-0 top-0 flex items-center ps-3.5 pointer-events-none">
									<svg class="w-4 h-4 text-gray-500" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
										<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"></path>
									</svg>
								</div>
								<input type="text" bind:value={$formData.informations.nom} aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full ps-10 p-2.5"  placeholder="Votre nom" required />
							</div>
							...
							<div class="relative">
								...
								<input type="text" bind:value={$formData.informations.prenom} aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full ps-10 p-2.5"  placeholder="Votre prénom" required />
							</div>
						</div>
						...
						<div class="relative">
							...
							<input type="text" id="phone" bind:value={$formData.informations.telephone} aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full ps-10 p-2.5"  placeholder="06 12 34 56 78" required />
						</div>
						...
						<div class="relative">
							...
							<input  type="email" bind:value={$formData.informations.email}  aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full ps-10 p-2.5"  placeholder="jean.dupont@gmail.com" required />
						</div>
...
<div class="w-full flex items-center justify-center bg-white py-10">
	<div class="w-full sm:w-11/12 xl:w-10/12 2xl:w-7/12 grid grid-cols-1 lg:grid-cols-2 p-5 gap-10">
		<div class="">
			<div class="">
				<ImgSlider></ImgSlider>
			</div>
		...
		<div class="w-full flex flex-col justify-center">
			<p class="text-lg font-bold">Simplifiez vos démarches et bénéficiez des subventions adaptées à vos projets.</p>
			<p>
				Le groupe Dumont vous accompagne dans tous vos projets de rénovation et travaux d’ampleur en vous permettant de bénéficier des aides de l’État, jusqu’à 100 % de financement. Spécialisés dans la gestion des démarches administratives et techniques, nous simplifions votre accès aux subventions telles que MonProjet Anah, les Certificats d’Économie d’Énergie (CEE) ou encore l’éco-prêt à taux zéro.			<p class="mb-5">
			</p>
			<p class="mb-5">Notre équipe prend en charge chaque étape de votre projet : analyse de votre éligibilité, constitution des dossiers, suivi des travaux avec des professionnels certifiés RGE, et obtention des financements adaptés. 	</p>
			<div class="flex">
				<a href="#test-elegibilite" type="button" class="flex items-center gap-3 shadow-lg text-green-500 border  border-green-500 px-5 py-1.5 rounded-full hover:bg-green-600 hover:text-white transition ease-in-out font-bold">
...
<div class="w-full flex flex-col bg-gray-50 items-center justify-center py-10">
	<div class="flex items-start sm:items-center">
		...
		<p class="text-xl sm:text-2xl font-semibold">Vos travaux d'isolation en 4 étapes  simples</p>
	</div>
	<div class="w-full sm:w-11/12 xl:w-10/12 2xl:w-7/12 grid grid-cols-1 md:grid-cols-2 p-8 gap-3">
		<ol class="relative text-gray-70 border-s border-green-400 ">                  
			<li class="mb-10 ms-6">            
				<span class="absolute flex items-center justify-center w-8 h-8 bg-green-500 rounded-full -start-4 ring-4 ring-white">
				...
				<h3 class="font-medium leading-tight">Je remplis le formulaire d'éligibilité</h3>
				<p class="text-sm">en 2 minutes</p>
			</li>
			<li class="mb-10 ms-6">
				<span class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -start-4 ring-4 ring-green-200">
					<svg class="w-3.5 h-3.5 text-green-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 16">
						<path d="M18 0H2a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2ZM6.5 3a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5ZM3.014 13.021l.157-.625A3.427 3.427 0 0 1 6.5 9.571a3.426 3.426 0 0 1 3.322 2.805l.159.622-6.967.023ZM16 12h-3a1 1 0 0 1 0-2h3a1 1 0 0 1 0 2Zm0-3h-3a1 1 0 1 1 0-2h3a1 1 0 1 1 0 2Zm0-3h-3a1 1 0 1 1 0-2h3a1 1 0 1 1 0 2Z"/>
					</svg>
				</span>
				<h3 class="font-medium leading-tight">Un technicien interne vous contacte</h3>
				<p class="text-sm">En moins de 48h </p>
			</li>
			<li class="mb-10 ms-6">
				<span class="absolute flex items-center justify-center w-8 h-8 bg-white rounded-full -start-4 ring-4 ring-green-200">
					<svg class="w-3.5 h-3.5 text-green-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 20">
						<path d="M16 1h-3.278A1.992 1.992 0 0 0 11 0H7a1.993 1.993 0 0 0-1.722 1H2a2 2 0 0 0-2 2v15a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2Zm-3 14H5a1 1 0 0 1 0-2h8a1 1 0 0 1 0 2Zm0-4H5a1 1 0 0 1 0-2h8a1 1 0 1 1 0 2Zm0-5H5a1 1 0 0 1 0-2h2V2h4v2h2a1 1 0 1 1 0 2Z"/>
					</svg>
				</span>
				<h3 class="font-medium leading-tight">Vous obtenez vos aides pour financer vos travaux</h3>
				<p class="text-sm">Selon votre éligibilité</p>
			</li>
			<li class="ms-6">
				<span class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -start-4 ring-4 ring-green-200">
					<svg class="w-3.5 h-3.5 text-green-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 20">
				...
				<h3 class="font-medium leading-tight">Mes travaux sont réalisés rapidement</h3>
				<p class="text-sm">Certifier RGE</p>
			</li>
		</ol>
...
<div class="w-full flex items-center justify-center py-20 object-cover bg-fixed" style="background-image: url(./bg-dumont.png);">
	<div class="w-full sm:w-11/12 xl:w-10/12 2xl:w-7/12 flex flex-col items-center justify-center">
		<h1 class="text-4xl text-center font-bold mb-2">Prêt à passer à l'isolation votre maison ?</h1>
...
<div class="w-full flex flex-col items-center justify-center bg-gray-50 py-20 gap-2 p-5">
	<p class="text-2xl font-semibold mb-5 text-center">Les principaux avantage de l'isolation par l'extérieur?</p>
	<div class="w-full sm:w-11/12 xl:w-10/12 2xl:w-7/12 grid grid-cols-2 sm:grid-cols-3 gap-3">
		<div class="flex flex-col justify-center items-center bg-white p-5 shadow-md rounded-xl">
			...
			<p class="text-lg text-center"><span class="font-bold text-green-500">Jusqu'a 63.000€</span> financer par les aides de l'Etat</p>
		</div>
		<div class="flex flex-col justify-center items-center bg-white p-5 shadow-md rounded-xl">
